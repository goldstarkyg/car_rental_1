				...
				<div class="clear_both">
					<form action="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjAdminOptions&amp;action=pjActionUpdate" method="post" class="form pj-form">
						...
						<div id="tabs">
							...
							<div id="tabs-1">
								<?php
								pjUtil::printNotice(__('infoConfirmationsTitle', true), __('infoConfirmationsBody', true)); 
								?>
								...
								<fieldset class="fieldset white">
									...
									<table class="pj-table" cellpadding="0" cellspacing="0" style="width: 100%">
										...
										<tbody>
											...
											for ($i = 0; $i < $count; $i++)
											{
												if ($tpl['arr'][$i]['tab_id'] == 5 && (int) $tpl['arr'][$i]['is_visible'] === 1 && (strpos($tpl['arr'][$i]['key'], 'admin') === false) && (strpos($tpl['arr'][$i]['key'], 'email') > -1))
												{
							...
							<div id="tabs-2">
								<?php
								pjUtil::printNotice(__('infoConfirmations2Title', true), __('infoConfirmations2Body', true)); 
								?>
								<?php if ((int) $tpl['option_arr']['o_multi_lang'] === 1 && count($tpl['lp_arr']) > 1) : ?>
								<div class="multilang"></div>
								<br/><br/>
								...
								<fieldset class="fieldset white">
									...
									<table class="pj-table" cellpadding="0" cellspacing="0" style="width: 100%">
										...
										<tbody>
											...
											for ($i = 0; $i < $count; $i++)
											{
												if (($tpl['arr'][$i]['tab_id'] == 5 && (int) $tpl['arr'][$i]['is_visible'] === 1 && strpos($tpl['arr'][$i]['key'], 'admin') >-1 && (strpos($tpl['arr'][$i]['key'], 'email') > -1)))
												{											
													$rowClass = NULL;
													$rowStyle = NULL;
								...
								<fieldset class="fieldset white">
									...
									<table class="pj-table" cellpadding="0" cellspacing="0" style="width: 100%">
										...
										<tbody>
											...
											for ($i = 0; $i < $count; $i++)
											{
												if (($tpl['arr'][$i]['tab_id'] == 5 && (int) $tpl['arr'][$i]['is_visible'] === 1 && strpos($tpl['arr'][$i]['key'], 'admin') >-1 && (strpos($tpl['arr'][$i]['key'], 'sms') > -1)))
												{											
													$rowClass = NULL;
													$rowStyle = NULL;
