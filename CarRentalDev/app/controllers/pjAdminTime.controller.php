...
class pjAdminTime extends pjAdmin
{
	...
	public function pjActionIndex()
	{
		...
		if ($this->isAdmin())
		{
			if (isset($_POST['working_time']))
			{
				...
				pjUtil::redirect(sprintf("%sindex.php?controller=pjAdminTime&action=pjActionIndex&id=" . $_POST['location_id'] . "&err=AWT01", PJ_INSTALL_URL));
			}else{
				$wt_arr = pjWorkingTimeModel::factory()
					->where('t1.location_id', $_GET['id'])
					->limit(1)
					->findAll()
					->getData();
	...
	public function pjActionCustom()
	{
		...
		if ($this->isAdmin())
		{
			if (isset($_POST['custom_time']))
			{
				...
				pjUtil::redirect($_SERVER['PHP_SELF'] . "?controller=pjAdminTime&action=pjActionCustom&id=".$location_id."&err=AWT02");
			}
	...
	public function pjActionGetDate()
	{
		...
		if ($this->isXHR() && $this->isLoged())
		{
			...
			$pjDateModel->where('t1.location_id', $location_id);
			...
			foreach($data as $k => $v)
			{
				...
				if($v['is_dayoff'] == 'F')
				{
					$v['start_time'] = !empty($v['start_time']) ? date("h:i A", strtotime($v['date'] . ' ' . $v['start_time'])) : '';
					$v['end_time'] = !empty($v['end_time']) ? date("h:i A", strtotime($v['date'] . ' ' . $v['end_time'])) : '';
				}else{
					$v['start_time'] = '';
					$v['end_time'] = '';
				}
	...
	public function pjActionUpdateCustom()
	{
		...
		if ($this->isAdmin())
		{
			if (isset($_POST['custom_time']))
			{
				...
				pjUtil::redirect($_SERVER['PHP_SELF'] . "?controller=pjAdminTime&action=pjActionCustom&id=".$_POST['location_id']."&err=AWT03");
			}
